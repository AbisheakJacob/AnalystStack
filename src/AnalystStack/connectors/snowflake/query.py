"""Execute SQL in Snowflake: Read and Write data"""
...
class QueryManager:
    """Handles read and write data operations to Snowflake.

    Every method runs through
    `AnalystStack.connectors.snowflake.client.SnowflakeClientWrapper`'s SQLAlchemy
    ...
    Args:
        client_wrapper: The `SnowflakeClientWrapper` supplying the SQLAlchemy engine.
    """

    def __init__(self, client_wrapper: SnowflakeClientWrapper):
    ...
    def execute_write(self, df: pd.DataFrame, schema: str, table_id: str, if_exists: str = "append") -> None:
        """Writes a DataFrame to a schema-qualified Snowflake table.
