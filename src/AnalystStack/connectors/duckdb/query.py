...
class QueryManager:
    """Handles read and write data operations to DuckDB.

    Every method runs through `AnalystStack.connectors.duckdb.client.DuckDBClientWrapper`'s
    ...
    Args:
        client_wrapper: The `DuckDBClientWrapper` supplying the SQLAlchemy engine.
    """

    def __init__(self, client_wrapper: DuckDBClientWrapper):
    ...
    def execute_write(self, df: pd.DataFrame, schema: str, table_id: str, if_exists: str = "append") -> None:
        """Writes a DataFrame to a schema-qualified DuckDB table.

        Args:
            df: The DataFrame to write.
            schema: The schema the destination table lives in.
